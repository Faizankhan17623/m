...
// console.log(organizer.attempts)
    ...
    //   Store full rejected data if status is rejected
...
return async (dispatch)=>{
   if(!token){
          ...
          if (!response.data.success) {
                    ...
                    // if user is verifiend attempts status editUntil Role ExperienceLevel

                    if(status === "approved"){
                      dispatch(setverification(true))
                      dispatch(setAttempts(response.attempts))
                      dispatch(setStatus(response.status))
                      dispatch(setEditUntil(response.editUntil))
                      dispatch(setRoleProfile(response.Role))
                      dispatch(setRoleExperience(response.ExperienceLevel))
                    }
                    ...
                    if(status === "locked"){
                      ...
                      dispatch(setStatus(response.status))
                      dispatch(setEditUntil(response.editUntil)) 
                      dispatch(setLockedUntill(response.lockedUntill)) 
                    }
